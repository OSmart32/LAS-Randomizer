...
from RandomizerCore.randomizer_data import VERSION, LOGIC_VERSION, DESC_VERSION
...
class SettingsUpdateProcess(QtCore.QThread):
    can_update = QtCore.Signal(bool)
    give_desc = QtCore.Signal(tuple)

    def __init__(self, parent=None, ver=DESC_VERSION):
        QtCore.QThread.__init__(self, parent)
        self.ver = ver
    ...
    def run(self):
        try:
            update_file =\
                lib.urlopen("https://raw.githubusercontent.com/Owen-Splat/LAS-Randomizer/scalable-window/RandomizerUI/Resources/descriptions.yml")
            web_version = float(update_file.readline().decode('utf-8').strip('#'))
            new_desc = update_file.read().decode('utf-8')

            if web_version > self.ver:
                self.give_desc.emit((web_version, new_desc))
                self.can_update.emit(True)
            else:
                self.can_update.emit(False)
